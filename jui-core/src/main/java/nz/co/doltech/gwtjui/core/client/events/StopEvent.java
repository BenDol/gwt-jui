...
public class StopEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, StopHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new StopEvent<T, H>(source, hash, nativeEvent));
    }

    private static final Type<StopHandler> TYPE = new Type<>();

    public static Type<StopHandler> getType() {
        return TYPE;
    }

    private StopEvent(T source, H hash, Event nativeEvent) {
        super(source, hash, nativeEvent);
    }

    @Override
    public Type<StopHandler> getAssociatedType() {
        return (Type) TYPE;
    }

    @Override
    protected void dispatch(final StopHandler handler) {
        handler.onStop(this);
    }
}
