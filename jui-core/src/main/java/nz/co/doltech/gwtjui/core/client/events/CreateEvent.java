...
public class CreateEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, CreateHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new CreateEvent<T, H>(source, hash, nativeEvent));
    }

    private static final Type<CreateHandler> TYPE = new Type<>();

    public static Type<CreateHandler> getType() {
        return TYPE;
    }

    private CreateEvent(T source, H hash, Event nativeEvent) {
    ...
    @Override
    protected void dispatch(final CreateHandler handler) {
