...
public class OutEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, OutHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new OutEvent<T, H>(source, hash, nativeEvent));
    }

    private static final Type<OutHandler> TYPE = new Type<>();

    public static Type<OutHandler> getType() {
        return TYPE;
    }

    private OutEvent(T source, H hash, Event nativeEvent) {
        super(source, hash, nativeEvent);
    }

    @Override
    public Type<OutHandler> getAssociatedType() {
        return (Type) TYPE;
    }

    @Override
    protected void dispatch(final OutHandler handler) {
        handler.onOut(this);
    }
}
