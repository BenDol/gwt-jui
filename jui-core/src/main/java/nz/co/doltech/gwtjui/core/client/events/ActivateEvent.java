...
public class ActivateEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, ActivateHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new ActivateEvent<T, H>(source, hash, nativeEvent));
    }

    private static final Type<ActivateHandler> TYPE = new Type<>();

    public static Type<ActivateHandler> getType() {
        return TYPE;
    }

    private ActivateEvent(T source, H hash, Event nativeEvent) {
    ...
    @Override
    protected void dispatch(final ActivateHandler handler) {
