...
public class UpdateEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, UpdateHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new UpdateEvent<T, H>(source, hash, nativeEvent));
    }

    private static final Type<UpdateHandler> TYPE = new Type<>();

    public static Type<UpdateHandler> getType() {
        return TYPE;
    }

    private UpdateEvent(T source, H hash, Event nativeEvent) {
    ...
    @Override
    protected void dispatch(final UpdateHandler handler) {
