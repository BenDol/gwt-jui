...
public class ReceiveEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, ReceiveHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new ReceiveEvent<T, H>(source, hash, nativeEvent));
    }

    private static final Type<ReceiveHandler> TYPE = new Type<>();

    public static Type<ReceiveHandler> getType() {
        return TYPE;
    }

    private ReceiveEvent(T source, H hash, Event nativeEvent) {
        super(source, hash, nativeEvent);
    }

    @Override
    public Type<ReceiveHandler> getAssociatedType() {
        return (Type) TYPE;
    }

    @Override
    protected void dispatch(final ReceiveHandler handler) {
