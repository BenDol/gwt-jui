...
public class StartEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, StartHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new StartEvent<T, H>(source, hash, nativeEvent));
    }

    private static final Type<StartHandler> TYPE = new Type<>();

    public static Type<StartHandler> getType() {
        return TYPE;
    }

    private StartEvent(T source, H hash, Event nativeEvent) {
    ...
    @Override
    protected void dispatch(final StartHandler handler) {
