 ...
 * on the drag of the resize handler.
 *
 * @author Ben Dol
 */
@SuppressWarnings({"unchecked"})
public class ResizeEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, ResizeHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new ResizeEvent<T, H>(source, hash, nativeEvent));
    }
    ...
    private ResizeEvent(T source, H hash, Event nativeEvent) {
