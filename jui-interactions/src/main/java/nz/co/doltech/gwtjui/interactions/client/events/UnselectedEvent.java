...
public class UnselectedEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, UnselectedHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new UnselectedEvent<T, H>(source, hash, nativeEvent));
    }

    private static final Type<UnselectedHandler> TYPE = new Type<>();

    public static Type<UnselectedHandler> getType() {
        return TYPE;
    }

    private UnselectedEvent(T source, H hash, Event nativeEvent) {
        super(source, hash, nativeEvent);
    }

    @Override
    public Type<UnselectedHandler> getAssociatedType() {
        return (Type) TYPE;
    }

    @Override
    protected void dispatch(final UnselectedHandler handler) {
