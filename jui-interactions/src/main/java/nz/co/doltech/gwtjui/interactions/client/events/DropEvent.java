 ...
 * on the droppable (based on thetolerance option).
 *
 * @author Ben Dol
 */
@SuppressWarnings({"unchecked"})
public class DropEvent<T extends JuiWrapper, H extends EventHash> extends JuiHashEvent<T, H, DropHandler> {

    public static <T extends JuiWrapper, H extends EventHash> void fire(T source, H hash, Event nativeEvent) {
        source.fireEvent(new DropEvent<T, H>(source, hash, nativeEvent));
    }
    ...
    private DropEvent(T source, H hash, Event nativeEvent) {
    ...
    @Override
    protected void dispatch(final DropHandler handler) {
        handler.onDrop(this);
    }
}
